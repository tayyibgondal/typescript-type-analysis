...
class AIBaselineAnalyzer:
    """
    Analyzer for AI PR baseline data.
    Focuses on any-related changes and advanced type features.
    Same filtering logic as human baseline.
    """
    ...
    def load_data(self, csv_file: str):
        """Load AI PR data from CSV."""
        print(f"Loading data from {csv_file}...")
        self.df = pd.read_csv(csv_file)
        print(f"Loaded {len(self.df):,} AI PRs")
        return self.df
    ...
    def filter_and_analyze_patches(self):
        ...
        # 3. Filter (same logic as human version)
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        # Calculate group statistics (all AI)
        by_group = {'AI': total_prs}
        
        # Calculate agent counts
        by_agent = {}
        if 'agent' in self.df.columns:
            by_agent = self.df['agent'].value_counts().to_dict()
        ...
        # Calculate group any statistics
        group_any_stats = {
            'AI': {
                'any_additions': int(self.df['any_additions'].sum()),
                'any_removals': int(self.df['any_removals'].sum()),
                'any_replacements': int(self.df['any_replacements'].sum()),
            }
        }
        
        # Calculate overall feature density (group level)
        group_feature_density = {}
        group_feature_density['AI'] = {}
        for col in self.ADVANCED_COUNT_COLS:
            total = float(self.df[col].sum())
            count = total_prs
            group_feature_density['AI'][col] = round(total / count if count > 0 else 0, 4)
    ...
    def export_results(self, output_file: str = 'ai_baseline_results.csv', 
                       summary_file: str = 'ai_baseline_summary.json'):
...
def main():
    print("="*80)
    print("AI Baseline PR Analysis")
    print("="*80)
    
    analyzer = AIBaselineAnalyzer()
    
    # Load data
    analyzer.load_data('agent_type_prs_final_type_related.csv')
