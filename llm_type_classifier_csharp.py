"""
AI-Powered Type-Related PR Classification Pipeline for C# (OpenAI GPT-4o-mini / Google Gemini)

This classifier analyzes C# PRs to determine if they are type-related using LLM analysis.
"""
...
class CSharpTypePRClassifier:
    """AI-powered C# type-related PR classifier with validation (OpenAI/Gemini)"""
    ...
    def create_classifier_prompt(self, pr_data: Dict) -> str:
        ...
        prompt = f"""You are an expert C# and software engineering analyst. Your task is to determine if a pull request is related to C# type changes, fixes, or improvements.
...
- C# files changed: {pr_data.get('cs_files_changed', 0)}

**Definition of TYPE-RELATED:**
A PR is type-related if it involves:
- Adding, modifying, or removing type annotations or declarations
- Fixing type errors or compilation issues
- Converting between nullable and non-nullable reference types
- Implementing or refactoring record types
- Pattern matching improvements or refactoring
- Generic type improvements or constraints
- Tuple usage or deconstruction
- Nullable reference type annotations (string?, int?, etc.)
- Null-forgiving operator usage (!)
- Init-only properties or required members
- Type casting (as, is operators)
- Interface implementation changes
- Type safety improvements
- Adding or modifying generic constraints (where clauses)
...
CRITICAL: BE LINIENT AND MOSTLY SAY YES EVEN IF YOU FIND A MINIMAL EVIDENCE.
...
Be thorough and accurate. Only classify as type-related if there's clear evidence."""
    ...
    def create_validator_prompt(self, pr_data: Dict, classification: Dict) -> str:
        ...
        prompt = f"""You are a senior code reviewer validating type-related PR classifications for C# code. 
...
- Title: {title}
- Description: {body}
- Files changed: {pr_data.get('cs_files_changed', 0)} C# files
...
Be critical and thorough in your validation."""
    ...
    async def _call_openai(self, prompt: str, temperature: float) -> Tuple[Dict, int]:
        ...
        response = await self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "You are a precise C# and software engineering expert. Always respond with valid JSON."},
    ...
    async def _call_gemini(self, prompt: str, temperature: float) -> Tuple[Dict, int]:
        ...
        def _sync_call():
            ...
            full_prompt = f"""You are a precise C# and software engineering expert. Always respond with valid JSON.
...
Respond ONLY with valid JSON matching the requested format."""
    ...
    async def classify_pr(self, pr_row: pd.Series, pr_index: int, total: int) -> Dict:
        ...
        pr_data = {
            ...
            'cs_files_changed': safe_get('cs_files_changed', 0),
        }
    ...
    async def classify_all_prs_async(self, 
                                    ...
                                    output_file: str = 'csharp_classified_type_prs.csv') -> pd.DataFrame:
        """
        ...
        log_progress("="*80)
        log_progress(f"Starting {self.provider.upper()} Type-Related PR Classification for C#")
    ...
    def classify_all_prs(self, 
                        ...
                        output_file: str = 'csharp_classified_type_prs.csv') -> pd.DataFrame:
        """
        Classify all PRs using AI agents (wrapper for async function)
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='AI-powered type-related PR classifier for C#')
    parser.add_argument('--input', default='csharp_type_prs_all_groups.csv',
                       help='Input CSV file with PRs')
    parser.add_argument('--output', default='csharp_classified_type_prs.csv',
    ...
    try:
        classifier = CSharpTypePRClassifier(
            model=args.model,
